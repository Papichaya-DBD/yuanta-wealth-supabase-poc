...
// PoC: port of yuanta-wealth-supabase-poc/single-weekly-market-calendar.html
// onto Webflow Cloud (Next.js). Same pattern as the other single-article
// ports: dynamic sections use React state + useEffect, not a raw injected
...
function fetchWeekRows(t: WTable, week: string) {
  ...
  return sbFetch(t.id, `select=*&week_slug=eq.${encodeURIComponent(week)}&limit=${limit}`).then((rows) =>
    (rows || []).map((row) => ({ values: row, _table: t }))
  );
}
function cardHtml(a: { values: Record<string, any>; _table: WTable }) {
  const t = a._table;
...
function monthlyCardHtml(a: { values: Record<string, any>; _table: WTable }) {
  const t = a._table;
  const src = THUMB[t.id] || FALLBACK_IMG;
  const url = t.path + (t.useId ? a.values.path || "" : (a.values.week_slug || "").trim());
  return (
    '<div class="col-6 col-lg-3">' +
    `<div class="insight-card" onclick="location.href='${url}'">` +
    `<div class="insight-card-img-wrap"><img class="insight-card-img" src="${src}" alt="" /></div>` +
...
const CSS = `
  ...
  .mc-table { width: 100%; border-collapse: collapse; font-family: 'Noto Sans Thai', sans-serif; font-size: 13px; margin: 8px 0; }
  .mc-table th { background: #0c244a; color: #fff; padding: 10px 14px; text-align: left; font-weight: 600; white-space: nowrap; }
  .mc-table td { padding: 8px 14px !important; border-bottom: 1px solid rgba(0,0,0,.07); vertical-align: middle; color: #1a2e44; }
  .mc-table tr:last-child td { border-bottom: none; }
  .mc-table .td-date { font-weight: 700; color: #0c244a; font-size: 12px; white-space: nowrap; vertical-align: top; padding-top: 12px !important; background: #f9f7f4; border-right: 1px solid rgba(0,0,0,.07); }
  .mc-table .td-type { white-space: nowrap; }
  .mc-type-macro    { background: #e8f0fb; color: #1a3a6b; padding: 2px 8px; font-size: 11px; font-weight: 600; display: inline-block; }
  .mc-type-micro    { background: #fef3cd; color: #92600a; padding: 2px 8px; font-size: 11px; font-weight: 600; display: inline-block; }
  .mc-type-earnings { background: #d4edda; color: #155724; padding: 2px 8px; font-size: 11px; font-weight: 600; display: inline-block; }
  .mc-type-global   { background: #e2d9f3; color: #4a235a; padding: 2px 8px; font-size: 11px; font-weight: 600; display: inline-block; }
  .mc-num { text-align: right; white-space: nowrap; }
  .mc-title { font-family: 'Noto Sans Thai', sans-serif; font-size: 16px; font-weight: 700; color: #0c244a; margin-bottom: 12px; padding-bottom: 10px; border-bottom: 2px solid #0c244a; display: inline-block; }
  td { padding: 10px 20px !important; }
  .article-content .mc-table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; }
  @media (max-width: 767px) { .article-content table { min-width: 600px; } }
  ...
  .week-modal-close { width: 32px; height: 32px; border: none; background: none; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #3d506e; border-radius: 50%; transition: background .15s; }
  .week-modal-close:hover { background: #f0f0f0; }
`;

export default function SingleWeeklyMarketCalendarPage() {
  const [row, setRow] = useState<Record<string, any> | null>(null);
  const [notFound, setNotFound] = useState(false);
  const [relatedHtml, setRelatedHtml] = useState<string | null>(null);
  const [weekModalHtml, setWeekModalHtml] = useState<string | null>(null);
  ...
  // Mobile scroll-hint for overflowing tables, once body content is in the DOM
  useEffect(() => {
    if (!row || window.innerWidth >= 768) return;
    document.querySelectorAll<HTMLElement>(".article-content table").forEach((table) => {
      const wrap = (table.closest('div[style*="overflow-x"]') as HTMLElement) || (table.parentElement as HTMLElement);
      if (!wrap || (wrap.nextElementSibling && wrap.nextElementSibling.classList.contains("mc-scroll-hint"))) return;
      if (table.scrollWidth <= wrap.clientWidth) return;
      const hint = document.createElement("p");
      hint.className = "mc-scroll-hint";
      hint.style.cssText = "font-family:'Noto Sans Thai',sans-serif;font-size:11px;color:#8f9aac;text-align:center;margin:6px 0 0;display:flex;align-items:center;justify-content:center;gap:6px;";
      hint.innerHTML = '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#8f9aac" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg>เลื่อนเพื่อดูข้อมูลเพิ่มเติม<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#8f9aac" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18l6-6-6-6"/></svg>';
  ...
  useEffect(() => {
    ...
    Promise.all(RELATED_TABLES.map((t) => fetchWeekRows(t, week))).then((results) => {
      const valid = ([] as any[]).concat(...results);
      if (valid.length) setRelatedHtml(valid.map(cardHtml).join(""));
    });
    Promise.all(RELATED_TABLES.map((t) => fetchWeekRows(t, week))).then((results) => {
      const valid = ([] as any[]).concat(...results);
      setWeekModalHtml(
        valid.length
          ? '<div class="row g-3">' + valid.map(weekModalCardHtml).join("") + "</div>"
          : '<p style="font-family:\'Noto Sans Thai\',sans-serif;font-size:14px;color:#3d506e;">ไม่มีบทความอื่นในสัปดาห์นี้</p>'
      );
    });
  }, [row]);
  ...
  useEffect(() => {
    if (!row) return;
    const monthSlug = row.week_slug ? row.week_slug.slice(0, 7) + "-01" : "";
    if (!monthSlug) return;
    Promise.all(
      MONTHLY_TABLES.map((t) => {
        const q = `select=*&week_slug=eq.${encodeURIComponent(monthSlug)}${t.limit ? `&limit=${t.limit}` : ""}`;
        return sbFetch(t.id, q).then((rows) => (rows || []).map((r) => ({ values: r, _table: t })));
      })
    ).then((results) => {
      const valid = ([] as any[]).concat(...results);
      if (!valid.length) return;
      setMonthlyHtml('<div class="row g-3">' + valid.map(monthlyCardHtml).join("") + "</div>");
    });
  }, [row]);
          ...
          <div className="insight-breadcrumb">
            <div className="container">
              <div className="col-12 col-lg-9 mx-auto">
                <ol>
                  <li><a href="https://landing.yuanta.co.th/wealth-home">Home</a></li>
                  <li className="sep">/</li>
              ...
              <div className="col-12 col-lg-9 mx-auto">
                ...
                <div className="article-cta-row">
                  <div className="article-cta-left">
                    ...
                    <a className="article-cta-btn" id="f-cta-link" href={`/wealth-weekly-report?week=${encodeURIComponent(row?.week_slug || "")}`} target="_blank" rel="noreferrer">ดูมุมมองเพิ่มเติม</a>
                  </div>
                    ...
                    <div className="container">
                      <div className="col-12 col-lg-9 mx-auto">
                        <div className="related-week-header-row">
                          ...
                          <span className="related-week-label" id="f-subtitle-2">{row?.page_subtitle || ""}</span>
                        </div>
      ...
      <div className="week-modal-overlay" id="weeklyModalOverlay" onClick={(e) => { if (e.target === e.currentTarget) closeWeeklyModalDirect(); }}>
        <div className="week-modal">
          <div className="week-modal-header">
            <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
              <span className="related-week-chip">Weekly</span>
              <span className="related-week-label" id="f-subtitle-3">{row?.page_subtitle || ""}</span>
            </div>
            <button className="week-modal-close" onClick={() => closeWeeklyModalDirect()}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>
          </div>
          <div className="week-modal-body" id="weekModalBody">
            {weekModalHtml ? (
              <div dangerouslySetInnerHTML={{ __html: weekModalHtml }} />
            ) : (
              <p style={{ fontFamily: "'Noto Sans Thai',sans-serif", fontSize: 14, color: "#3d506e" }}>กำลังโหลด...</p>
            )}
          </div>
        </div>
      </div>

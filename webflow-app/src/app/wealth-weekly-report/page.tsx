...
// PoC: port of yuanta-wealth-supabase-poc/weekly-pdf.html onto Webflow Cloud.
// Standalone redirect/popup page — production has no site nav/footer here
// either (not extending the base layout), kept that way for fidelity. No SSO
// gate (dropped per the agreed PoC scope decision).
...
export default function WeeklyReportPage() {
  ...
  useEffect(() => {
    ...
    sbFetch("weekly_pdf", `select=*&week_slug=eq.${encodeURIComponent(week)}&limit=1`).then((rows) => {
      if (!rows.length) {
        setView("error");
        return;
      }
      const row = rows[0];
      setTitle(row.hs_name || row.week_slug || "");
    ...
    <>
      ...
      {view === "loading" && (
        <div className="popup-card" id="loadCard">
          ...
          <p className="popup-label">Weekly Report</p>
      ...
      {view === "error" && (
        <div className="popup-card" id="errorCard">
          ...
          <p className="popup-error-desc">ยังไม่มีบทวิเคราะห์สำหรับสัปดาห์นี้<br />กำลังกลับไปหน้าหลัก…</p>
